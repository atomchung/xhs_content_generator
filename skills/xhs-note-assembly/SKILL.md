...
- 输出标题、正文、图组任务
- 默认交付 `publish-ready copy`，必要时再附 `research-safe draft`
- 从成稿中提取封面结构
...
- 默认直接给用户 `publish-ready copy`
- 只有在用户明确要保留研究口径，或事实仍待核时，再额外附 `research-safe draft`
- 不要把研究稿语气直接当成发布稿交出去
...
- 如果是商业题，默认先写 `结果 / 结论 + 最大数字 + 最短归因`，不要先用问题句把读者挂着
...
3. **动作**：先用一句话说清人物或画面正在发生什么
4. **冲突**：确认这张封面到底在放大哪一个结果、张力或反差
5. **场景**：从正文背景信息中提取场景符号
