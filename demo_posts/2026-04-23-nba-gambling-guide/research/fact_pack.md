...
## v3 Changelog（2026-04-23 用户要求重排结构）

- 用户要求顺序重排：**① 假球事件 → ② 什么是单人赌球 → ③ 为什么防不住**
- 修正硬错：Jontay Porter 当季效力 **猛龙（Toronto Raptors）**，不是公牛
- 正文主用 Porter 案（首位因赌博终身禁赛）；Beasley 留作对照素材
- 正文字数从 305 撑到 ~585（explainer 型，主动选择）
- 引用延伸研究 `extension_questions.md` 的 Q2（谁开的数据链）补进正文二段
...
## 一、核心案件（本篇正文主用 Porter，Beasley 作对照）

### A. Jontay Porter 假球案（正文一段主用）

**修正前稿错误**：此人当季效力 **猛龙（Toronto Raptors）**，不是公牛。two-way 转标准合同。Michael Porter Jr. 的弟弟。

两场可疑比赛的 timeline：

- ✅ 2024 年 1 月 26 日 vs **快船**：开场约 3 分钟以「眼睛不适」退场
- ✅ 2024 年 3 月 20 日 vs **国王**：开场约 3 分钟以「身体不适」退场
- ✅ 共谋下注模式：**同一位用户把 8 万美元押成一串 parlay**，全部是 Porter 个人数据的 under（低于某个得分 / 低于某个篮板 / 低于某个三分），预估派彩约 **$1.1M**
- ✅ **DraftKings 监测到异常后冻结了那笔 110 万派彩**，没付
- ✅ 诚信监控商当晚预警 → NBA 内部调查 → FBI / 联邦检察官介入
- ✅ 2024 年 4 月 17 日：NBA 公告**终身禁赛**，联盟史上第一位因下注操纵比赛被永久封杀的球员
- ✅ 2024 年 7 月 10 日：**对联邦认罪**，罪名是 wire fraud conspiracy，最高可判 20 年，预估实际量刑 **约 41-51 个月**
- 🔲 最新量刑进度（发帖前再核，2025 年底 / 2026 年初应有宣判）
- ✅ 他本人违规下注金额：13 笔 NBA 比赛共约 $54,094（前篇 fact_pack 已用）

这一场为什么是本篇正文开场：
- 这是 **NBA 史上第一个被司法确认的「假球」**，不是「嫌疑」
- 事件链条干净：3 分钟 → 异常下注 → 监控报警 → 派彩冻结 → FBI → 终身禁赛 → 认罪
- 能让完全不懂 NBA 的读者在一段内明白「单人赌球怎么被操纵 + 怎么被抓」

### B. Malik Beasley 对照案（本篇正文不用，但作素材留底）

这是 prop 机制**没被成功操纵**的教科书对照：
...
这一场的价值：
- 异常资金流入被监控商实时抓到（所以才会成为调查线索）
- 球员**实际表现并没有配合**那个低档市场（所以没被禁赛）
- 本篇正文用 Porter 后，Beasley 留给第三篇或图组做对照（「操纵成功 vs 操纵失败 vs 没操纵只是巧合」三分类）
